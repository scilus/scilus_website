...
{\bf La date de remise sera déterminée en classe.}
% Exercice 1.
% g0(n) = [0.5 1 1 0.5].
...
\item {\bf TFD et Convolution [20 pts]}
  ...
  \item En Python, construiser
    et illustrer la Gaussienne 1D et sa TF de la partie (a), utilisant
    un $\sigma = 6$ et 1024 points discrets de temps $t$. 
  
\vspace{0.5cm}  
  \item En utilisant la \emph{fft} de Python, calculer et illustrer sa
    transformée de Fourier $G(f)$. \\
    Visualiser le résultat et assurez-vous que le résultat théorique en b)
    correspond à votre résultat trouvé numériquement.
...
\vspace{0.5cm}  
  \item Charger le signal 1D 'piece-regular'
    ...
    \cdot$rand(1024,1)). Si $\tau=0.1$, vous ajoutez 10\% de bruit. 
...
\begin{figure}[!t]
  \begin{center}
    \includegraphics[width=9cm]{Sinusoidal.jpg} 
